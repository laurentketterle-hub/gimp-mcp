...
class MockBackend:
    ...
    def close_image(self, image_id: str) -> dict[str, Any]:
        ...
        self._cleanup_selection(image_id)
    ...
    def selection_rect(self, image_id: str, x: int, y: int, width: int, height: int,
        ...
        note = "Feathering not supported in mock mode" if feather else ""
        return {"ok": True, "image_id": image_id, "selection": sel, "note": note or None}

    def selection_none(self, image_id: str) -> dict[str, Any]:
        """Clear the current selection."""
        if image_id in self._selections:
            del self._selections[image_id]
        return {"ok": True, "image_id": image_id, "selection": None}

    def selection_fill(self, image_id: str, color: str = "#000000") -> dict[str, Any]:
        """Fill the current selection with a color."""
        sel = self._get_selection(image_id)
        if sel is None:
            return {"ok": False, "error": "No active selection. Use gimp_selection_rect first."}
        if sel["type"] != "rectangle":
            return {"ok": False, "error": f"Unsupported selection type: {sel['type']}"}
        return self._apply(
            image_id,
            ops.selection_fill,
            x=sel["x"],
    ...
    def _cleanup_selection(self, image_id: str) -> None:
        """Remove selection when image is closed."""
        self._selections.pop(image_id, None)
    ...
    def new_layer(self, image_id: str, name: str = "New Layer") -> dict[str, Any]:
        ...
        try:
            self._load(image_id)  # validate image exists
