...
def selection_fill(im: Image.Image, x: int, y: int, width: int, height: int, color: str = "#000000") -> Image.Image:
    """Fill a rectangular selection area with solid color."""
    out = im.copy()
    if out.mode not in ("RGB", "RGBA"):
        out = out.convert("RGBA")
    draw = ImageDraw.Draw(out)
    x0, y0 = int(x), int(y)
    x1, y1 = x0 + max(1, int(width)), y0 + max(1, int(height))
    draw.rectangle((x0, y0, x1, y1), fill=color)
    return out


def selection_stroke(im: Image.Image, x: int, y: int, width: int, height: int,
                     stroke_width: int = 2, color: str = "#000000") -> Image.Image:
    """Stroke (draw border) around a rectangular selection."""
    out = im.copy()
    if out.mode not in ("RGB", "RGBA"):
        out = out.convert("RGBA")
    draw = ImageDraw.Draw(out)
