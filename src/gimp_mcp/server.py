...
def gimp_pipeline(image_id: str, steps_json: str) -> str:
    ...
    erase_rect, fill_rect, remove_background, trim, pad, border, opacity, selection_fill, selection_stroke.
    """
...
@mcp.tool()
def gimp_selection_rect(image_id: str, x: int, y: int, width: int, height: int,
                        feather: bool = False, feather_radius: float = 5.0) -> str:
    """Create a rectangular selection on the image. Fill/stroke with gimp_selection_fill / gimp_selection_stroke."""
    return _j(get_backend().selection_rect(image_id, x, y, width, height, feather, feather_radius))


@mcp.tool()
def gimp_selection_none(image_id: str) -> str:
    """Clear the current selection."""
    return _j(get_backend().selection_none(image_id))


@mcp.tool()
def gimp_selection_fill(image_id: str, color: str = "#000000") -> str:
    """Fill the current selection with a solid color."""
    return _j(get_backend().selection_fill(image_id, color))


@mcp.tool()
def gimp_selection_stroke(image_id: str, width: int = 2, color: str = "#000000") -> str:
    """Stroke (draw border) around the current selection."""
    return _j(get_backend().selection_stroke(image_id, width, color))
